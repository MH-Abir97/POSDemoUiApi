app.controller("ReviseCommercialInvoiceController", function ($scope, hotkeys, $cookieStore, $http, $filter, $window) {
    ...
    //    $http({
    //        url: "/BankAccount/GetBankAccountByTypeAndRefId?AccountFor=Exporter&AccountRefId=" +
    //            1,
    //        method: "GET",
    //        headers: { 'Content-Type': "application/json" }
    //    }).success(function (data) {
    ...
    $scope.LoadCommercialInvoice = function (password) {
        ...
        $http({
            ...
            if (aCommercialInvoice.length > 0) {
                ...
                ////////////////////////////
                //var invoiceIdArr = [];
                //invoiceIdArr = aCommercialInvoice[0].InvoiceIds.split(',');
    ...
    //            var isExpDateExist = isNaN(data[0].ExpDate);
    //            var isCommercialInvoiceDateExist = isNaN(data[0].CommercialInvoiceDate);
    //            var isLcScDate = isNaN(data[0].LcScDate);
    //            var isMasterDate = isNaN(data[0].LcScDate);
    //            if (isExpDateExist) {
    //                var res1 = data[0].ExpDate.substring(0, 5);
    //                if (res1 == "/Date") {
    //                    var parsedDate1 = new Date(parseInt(data[0].ExpDate.substr(6)));
    //                    var date1 = ($filter('date')(parsedDate1, 'MMM dd, yyyy')).toString();
    //                    data[0].ExpDate = date1;
    //                }
    //            }
    //            if (isCommercialInvoiceDateExist) {
    //                var res2 = data[0].CommercialInvoiceDate.substring(0, 5);
    //                if (res2 == "/Date") {
    //                    var parsedDate2 = new Date(parseInt(data[0].CommercialInvoiceDate.substr(6)));
    //                    var date2 = ($filter('date')(parsedDate2, 'MMM dd, yyyy')).toString();
    //                    data[0].CommercialInvoiceDate = date2;
    //                }
    //            }

    //            if (isLcScDate == true) {
    //                var res3 = data[0].LcScDate.substring(0, 5);
    //                if (res3 == "/Date") {
    //                    var parsedDate3 = new Date(parseInt(data[0].LcScDate.substr(6)));
    //                    var date3 = ($filter('date')(parsedDate3, 'MMM dd, yyyy')).toString();
    //                    data[0].LcScDate = date3;
    //                }
    //            }
    //            if (isMasterDate == true) {
    //                var res4 = data[0].MasterContactDate.substring(0, 5);
    //                if (res4 == "/Date") {
    //                    var parsedDate4 = new Date(parseInt(data[0].MasterContactDate.substr(6)));
    //                    var date4 = ($filter('date')(parsedDate4, 'MMM dd, yyyy')).toString();
    //                    data[0].MasterContactDate = date4;
    //                }
    //            }
    //            $scope.exp_CommercialInvoice = data[0];
    //            GetAllActiveCompany();
    //            $scope.ddlPaymentProcessType = { PaymentProcessType: data[0].PaymentProcessType };
    //            $scope.ddlImporter = { "CompanyId": data[0].CompanyId };
    //            $scope.GetImporterBankByRefId(data[0].CompanyId, 'Customer');
    //            $scope.ddlImporterBank = { "BankAccountId": data[0].ImporterBankId };
    //            GetCommercialInvoiceInfoByCiId(data[0].CommercialInvoiceId);
    //            LoadInvoice($scope.ddlImporter, data[0].CommercialInvoiceId);
    //            GetItemByCI(data[0].CommercialInvoiceId);
    ...
    //    $http({
    //        url: '/ExpPackingInfo/GetAllPackingInfo',
    //        method: 'GET',
    //        headers: { 'Content-Type': 'application/json' }
    //    }).success(function (data) {
    //        $scope.PackingInfoList = data;
    //    });
    //}
    ...
    //            objExtra.SalesOrderId = adata.SalesOrderId;
    //            //objExtra.CIPackingInfold = adata.CIPackingInfold;
    //            objExtra.Qty = adata.Quantity;
    //            objExtra.SubCategoryName = adata.SubCategoryName;
    //            objExtra.itemW8 = adata.Quantity * adata.PackageWeight;
    //            objExtra.crtnW8 = objExtra.noOfCrtn * adata.ContainerWeight;
