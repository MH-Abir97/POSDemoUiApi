app.controller("ProformaInvoiceController",
    function ($scope, hotkeys, $cookieStore, $http, $filter, $window, $route) {
        ...
        function invoiceTypeChange() {
            if ($scope.exportInvoice.InvoiceType == "PI") {
                $scope.TermsAndConditionList =
                    "<ol><li>Payment: Letter of Credit&nbsp;<b> 90 days&nbsp;</b>From the date of Delivery Challan to be opened in favor of Retail Technologies Ltd.</li><li>Payment Should be made in U.S Dollar through LC.</li><li>Partial Shipment Allowed.</li><li>&nbsp;Quantity &amp; Value may vary +/- 10% of total Quantity &amp; Value of the Proforma Invoice.</li><li>Delivery Challan Should be treated as transport/Truck Challan.</li><li>Maturity date should be calculated from the date of goods delivery Challan.</li><li>All Banking Charges inside openers Bank counter on account of opener and outside openers bank counter on account of beneficiary.</li><li>Payment after Export Realization clause not allowed in the LC.</li><li>LC must incorporate delivery validity 30 days from the date of LC.</li><li>Presentation period: 15 days from the date of delivery.</li><li>L/C should be freely negotiable.</li><li>PI Validity 65 days from the date of issue.</li><li>Discrepancy charge should be mentioned between 25-30 Dollars.<br></li></ol>";
                $(".summernote").summernote("code", $scope.TermsAndConditionList);
            } else if ($scope.exportInvoice.InvoiceType == "SC/TT") {
                $scope.TermsAndConditionList = $scope.TermsAndConditionListForTT;
                $(".summernote").summernote("code", $scope.TermsAndConditionList);
            } else if ($scope.exportInvoice.InvoiceType == "SC/FDD") {
                    ...
                    if ($(e.target).is("#mofiz tbody tr td")) {
                        ...
                        $("#right").unbind().click(function () {
                            ...
                            $("#mofiz tfoot tr td:eq(" + column + ")").after($(
                                '<td  style="text-align:right;padding-right: 4px;" class="col" contentEditable="true"></td>'));
                            //$('#mofiz tfoot tr').each(function () {
                            //    //$('td:first').after($("<td>Value</td>"));
                            //    $(this).children('td:eq(' + column + ')').after($('<td id="tright" contentEditable="true"></td>'))
                            //});
                            //$("#mofiz").colResizable({
                            //    liveDrag: true,
                            //    resizeMode: 'flex',
        ...
        function GetAllProformaInvoiceList() {
            $http({
                ...
                $scope.total_count = $scope.invoiceList.length; 
            });
        }
        ...
        function GetEmployeeEmailByDocumentRef() {
            $http({
                url: "/Employee/GetEmployeeEmailByDocumentRef?refEmployeeId=" +
                    ...
                    "&invoiceType=" +
                    $scope.exportInvoice.InvoiceType,
                method: "GET",
                headers: { 'Content-type': "application/json" }
            }).success(function (data) {
                if (data.length > 0) {
                        ...
                        angular.forEach($scope.invoiceDetailList,
                            function (invoice) {
                                if (invoice.IsOverride == true) {
                                    invoice.DescriptionTwo = "override";
                                    invoice.DescriptionOne = invoice.DescriptionOverride;
                                } else {
                                    invoice.DescriptionTwo = "";
                                }
                            });
...
/*        $scope.GetItemValueForAmountCalculation = function (itemName, row, index) {
            ...
            var item;
            if (itemType != Number && row.length - 3 >= index) {
                var itemNameFiltered = itemName.toString().replace(/\D/g, "");
                item = Number(itemNameFiltered);
            }
            else {
                item = itemName;
            }
            if ($scope.insertItemCount == undefined) {
                ...
                if ($scope.distCount == 4) {
                    $scope.ItemTableFooter[$scope.ItemTableFooter.length - 3] = ($scope.QtySumForItem).toFixed(2);
                    $scope.ItemTableFooter[$scope.ItemTableFooter.length - 1] = ($scope.AmountSumForItem).toFixed(2);
                    $scope.distCount = 1;
                }
            }
        };*/
        ...
        //    var isConverted = 0;
        //    var hasDot;
        //    row[row.length - 2] = Number(row[row.length - 2]);
        //    row[row.length - 1] = Number(row[row.length - 1]);
        ...
        //    var hasBraStart = itemName.indexOf("(");
        //    var hasBraEnd = itemName.indexOf(")");

        //    if (hasBraStart != -1 || hasBraEnd != -1) {
        ...
        //    }
        //    var item;
        //    if (row.length - 3 <= index) {
        //        var itemType = typeof (itemName);
        //        var convertToArr = Array.from(itemName);
        //        var itemNameDot;
        //        if (convertToArr[convertToArr.length - 1] == '.') {
        //            itemNameDot = itemName;
        //            itemNameDot = itemNameDot.slice(0, convertToArr.length - 1);

        //        }
        //        if (itemNameDot != undefined) {
        //            if (itemType != 'number') {
        //                var itemNameFiltered = itemName.toString().replace(/\D/g, "");
        //                item = Number(itemNameFiltered);
        //            }
        ...
        //            }
        //        }
        //        else if (convertToArr[length - 1] != '.' && hasDot == -1) {
        //            if (itemType != 'number') {
        //                var itemNameFiltered = itemName.toString().replace(/\D/g, "");
        //                item = Number(itemNameFiltered);
        //            }
        ...
        //            }
        //        }
        //        itemNameDot = undefined;

        //    }

        //    if (row.length - 1 != index) {
        //        $scope.QtySumForItem -= Number(row[row.length - 3]);
        //        $scope.AmountSumForItem -= Number(row[row.length - 1]);
        //        if (index >= (row.length - 3)) {
        //            row[index] = item;
        //        }
        //        else {
        //            row[index] = item;
        //        }
        //        var qtyConvert = Number(row[row.length - 3]);
        //        var amountConvert = Number(row[row.length - 2]);
        ...
        //    $scope.ItemTableFooter[$scope.ItemTableFooter.length - 3] = Number($scope.QtySumForItem);
        ...
        function GetFactoryList() {
            $scope.FactoryList = [{ FactoryId: 1, Factory: "EPZ" }, { FactoryId: 2, Factory: "Uttara" }];
            //$http({
            ...
            if ($scope.exportInvoice.InvoiceDate != "") {
                ...
                $http({
                    ...
                    if ($scope.InvoiceNoTemp == "") {
                        if ($scope.exportInvoice.InvoiceType == "SC/TT" || $scope.exportInvoice.InvoiceType == "SC/FDD") {
        ...
        //        $http({
        //            url: "/ExpInvoice/GetInvoiceNo?InvoiceDate=" + from + "&factory=" + $scope.ddlFactory.ExporterAddress2,
        //            method: "GET",
        //            headers: { 'Content-Type': "application/json" }
        //        }).success(function (data) {
        //            if ($scope.exportInvoice.InvoiceType == "SC/TT" || $scope.exportInvoice.InvoiceType == "SC/FDD") {
        //                $scope.exportInvoice.InvoiceNo = $scope.ddlFactory.ExporterAddress2 === "EPZ"
        //                    ? ("RTLE/" + finYear + "/SC-" + data)
        //                    : ("RTL/" + finYear + "/SC-" + data);
        //            }
        //            else {
        //                $scope.exportInvoice.InvoiceNo = $scope.ddlFactory.ExporterAddress2 === "EPZ"
        //                    ? ("RTLE/" + finYear + "/PI-" + data)
        //                    : ("RTL/" + finYear + "/PI-" + data);
        //            }
                    
        //        });
        //    }
        //    if ($scope.ddlFactory.ExporterAddress2 === "EPZ") {
        //        $scope.ddlExporterBank = { BankAccountId: 2 };
        //        $scope.exportInvoice.ExporterBankId = 2;
        //    }
        //    else {
        ...
        $scope.searchProformaInvoice = function () {
            ...
            $http({
                url: "/ExpInvoice/ExpInvoiceGetForEdit?fromDate=" +
                    fromDate +
                    "&toDate=" +
                    toDate +
                    "&companyId=" +
            ...
            $http({
                ...
                if (data.length) {
                    angular.forEach(data,
                        function (adata) {
                            adata.PreAmountOfLcCoppyItem = adata.UnitPrice;
                            //if (adata.DescriptionTwo === 'override') {
                            //    adata.IsOverride = true;
                            //}                         
                            $scope.invoiceDetailList.push(adata);
                            QuantityAndAmountSum();
        ...
        $scope.openReport = function (invoiceId) {
            event.stopPropagation();
            $window.open(
                "http://43.224.119.250/Reports_MSSQLSERVER2016/report/Reports/Export/Rpt_ProformaInvoice?InvoiceId=" +
                invoiceId);
        };
        ...
        $scope.getPackingInfo = function () {
            ...
            if ($scope.itemFlag == true) {
                ...
                $scope.margeProduct = $scope.invoiceDetailList.reduce((r, { Amount, ContainerId, ContainerSize, ContainerWeight, DescriptionOne, DescriptionTwo, HsCode, InvoiceDetailId, InvoiceId, ItemId, IdenticalFlag, ItemName, OrderUnitId, PackageId, PackagePerContainer, PackageWeight, Quantity, SalesOrderId, SubCategoryId, SubCategoryName, UnitId, UnitPerPackage, UnitPrice }) => {
                    var temp = r.find(o => o.ItemId === ItemId);
                    if (!temp) {
                        r.push({ Amount, ContainerId, ContainerSize, ContainerWeight, DescriptionOne, DescriptionTwo, HsCode, InvoiceDetailId, InvoiceId, ItemId, IdenticalFlag, ItemName, OrderUnitId, PackageId, PackagePerContainer, PackageWeight, Quantity, SalesOrderId, SubCategoryId, SubCategoryName, UnitId, UnitPerPackage, UnitPrice });
                    } else {
        ...
        function SaveItem(status) {
            ...
            var where = "$.FirstAttribute.toLowerCase()=='" +
                $scope.ad_Item.ItemDescription.toLowerCase() +
                "' && $.ItemName.toLowerCase()=='" +
                $scope.ad_Item.ItemName.toLowerCase() +
                "'";
            if ($scope.ad_Item.ItemId > 0)
            ...
            if (itemCombinationExists != null || !angular.isUndefined(itemCombinationExists)) {
                alertify.log($scope.ad_Item.ItemName +
                    " with Description " +
                    $scope.ad_Item.ItemDescription +
                    " already exists",
                    "error",
            ...
            var subCategory = Enumerable.From($scope.SubcategoryList)
                .Where("$.SubCategoryId == " + $scope.ddlSubCategory.SubCategoryId).FirstOrDefault();
            ...
            $http.post("/Item/SaveNew", parms).success(function (data) {
                if (data > 0) {
                    ...
                    $http({
                        ...
                        var aCombination = Enumerable.From($scope.AllCombinationlist).Where("$.ItemAddAttId==" + data)
                            .FirstOrDefault();
        ...
        function restMergeInvoiceDetails() {
            //$http({
            //    url: "/ExpInvoice/InvoiceDetailDeleteByInvoiceId?invoiceId=" + $scope.invoiceDetailList[0].InvoiceId,
            //    method: "GET",
            //    headers: { 'Content-Type': "application/json" }
            //}).success(function (data) {
            $scope.ItemCategory = [];
        ...
        function getItemDetailModifiedDataForUpdate(invoiceId) {
            ...
            $http({
                ...
                //console.log($scope.ItemTableDataRow);
                $scope.ItemTableFooter = [];
                ...
                //for (var i = 0; i < $scope.ItemTableHeaders.length; i++) {
                //    if ($scope.ItemTableHeaders[i] === "Qty/Rolls" || $scope.ItemTableHeaders[i] === "Amount") {
                //        $scope.ItemTableFooter.push(0);
                //    } else {
                //        $scope.ItemTableFooter.push("");
                //    }
        ...
        function GetInvoicePaged(curPage) {
            ...
            if ($scope.SearchInvoiceAndCompanyName != undefined && $scope.SearchInvoiceAndCompanyName != "" && $scope.FromDate != "" && $scope.ToDate != "") {
                SearchCriteria = "([InvoiceDate] between '" + $scope.FromDate + "' and '" + $scope.ToDate + "') and ([InvoiceNo] LIKE '%" + $scope.SearchInvoiceAndCompanyName + "%' OR [CompanyNameBilling] LIKE '%" + $scope.SearchInvoiceAndCompanyName + "%')";
                ...
                SearchCriteria = "[InvoiceNo] LIKE '%" + $scope.SearchInvoiceAndCompanyName + "%' OR [CompanyNameBilling] LIKE '%" + $scope.SearchInvoiceAndCompanyName + "%'";
                //alert("Name Success!!!!!");
            }
