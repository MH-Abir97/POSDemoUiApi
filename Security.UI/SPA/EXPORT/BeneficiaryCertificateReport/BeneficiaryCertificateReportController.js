...
app.controller("BeneficiaryCertificateReportController", function ($scope, $cookieStore, $http, $filter, $window) {
    ...
    function GetCIMasterByCIid() {
        $http({
            ...
            $scope.CommercialInvoiceMasterList[0].Amount = parseFloat($scope.CommercialInvoiceMasterList[0].Amount).toFixed(2);
            ...
            var arrPONo = $scope.CommercialInvoiceMasterList[0].PONo.split(",");
            var arrPODate = $scope.CommercialInvoiceMasterList[0].PODate.split(",");
    ...
    function GetTableHtmlForCi() {
        $http({
            ...
            var margeSubTable = '<table id="ciTable" border="1" class="JCLRFlex" style="width: 100% !important; font-size: .9em; font-family: Calibri; color:#000000; text-align: center; margin:0; padding:0;"> <thead>' + SubTable[1];
            ...
            if (remainderQty > 0) {
                jqueryFun.prevObject.prevObject.prevObject[$('#ciTable tbody tr').length - 1].children[$('#ciTable thead tr th').length - 3].innerHTML = (itemQuantity).toFixed(2);
            }
            else {
                jqueryFun.prevObject.prevObject.prevObject[$('#ciTable tbody tr').length - 1].children[$('#ciTable thead tr th').length - 3].innerHTML = parseItemQuantity;
            }
