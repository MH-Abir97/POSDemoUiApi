app.controller("ReviseProformaInvoiceController", function ($scope, hotkeys, $cookieStore, $http, $filter, $window) {
    ...
    //                if ($scope.isLoadItemCategory) {
    //                    angular.forEach($scope.SalesOrderList, function (aSalesOrder) {
    //                        aSalesOrder.disabled = true;
    //                    });
    //                }
    //                if (!$scope.invoiceDetailList.length) {
    //                    angular.forEach($scope.SalesOrderList, function (aSalesOrder) {
    //                        aSalesOrder.disabled = false;
    //                        aSalesOrder.IsChecked = false;
    //                        if ($scope.isLoadItemCategory) {
    //                            aSalesOrder.disabled = true;
    //                            aSalesOrder.IsChecked = true;
    //                        }
    //                    });
    //                }
    //                //if (!$scope.isLoadItemCategory) {
    ...
    function GetEmployeeEmailByDocumentRef() {
        $http({
            url: '/Employee/GetEmployeeEmailByDocumentRef?refEmployeeId=' + $scope.exportInvoice.RefEmployeeId + '&invoiceType=' + $scope.exportInvoice.InvoiceType,
            method: 'GET',
            headers: { 'Content-type': 'application/json' }
        }).success(function (data) {
            if (data.length > 0) {
            ...
            if ($scope.saveButtonLabel == 'Submit Amend') {
    ...
    //    $http({
    //        url: "/BankAccount/GetBankAccountByTypeAndRefId?AccountFor=Exporter",
    //        method: "GET",
    //        headers: { 'Content-Type': "application/json" }
    //    }).success(function (data) {
    ...
    $scope.GetInvoiceAndFactoryList = function () {
        $scope.InvoiceTypeList = [{ InvoiceTypeId: 1, InvoiceType: "PI" }, { InvoiceTypeId: 2, InvoiceType: "SC/TT" }, { InvoiceTypeId: 3, InvoiceType: "SC/FDD" }];
    ...
    //        var yearTwoDigit = Number(year.slice(-2));
    //        var finYear = month < 3 ? ((yearTwoDigit - 1) + '-' + yearTwoDigit) : (yearTwoDigit + '-' + (yearTwoDigit + 1));

    //        var monthString = month < 10 ? ('0' + month) : ('' + month);
    //        var from = year + "-" + monthString + "-" + date;

    //        $http({
    //            url: '/ExpInvoice/GetInvoiceNo?InvoiceDate=' + from + '&factory=' + $scope.ddlFactory.Factory,
    //            method: 'GET',
    //            headers: { 'Content-Type': 'application/json' }
    //        }).success(function (data) {
    //            if ($scope.exportInvoice.InvoiceType == "SC/TT" || $scope.exportInvoice.InvoiceType == "SC/FDD") {
    //                $scope.exportInvoice.InvoiceNo = $scope.ddlFactory.Factory === "EPZ"
    ...
    //        });
    //    }
    //    if ($scope.ddlFactory.Factory === "EPZ") {
    //        $scope.ddlExporterBank = { BankAccountId: 2 };
    //        $scope.exportInvoice.ExporterBankId = 2;
    //    }
    //    else {
    ...
    //$scope.resetMergeInvoiceDetails = function () {
    //    $scope.totalAmount = 0;
    ...
    //        $scope.ItemCategory = [];
    //        $scope.invoiceDetailList = [];
    //        $scope.isLoadItemCategory = false;
    //    });
    //}
    ...
    //            if (adata.PackagePerContainer != 0) {
    //                noOfCrtn1 = adata.Quantity / adata.PackagePerContainer;
    //                obj.noOfCrtn = Math.ceil(noOfCrtn1);
    //            } else {
    //                if (adata.UnitPerPackage != 0) {
    //                    noOfCrtn1 = adata.Quantity / adata.UnitPerPackage;
    //                    obj.noOfCrtn = Math.ceil(noOfCrtn1);
    ...
    //            obj.SalesOrderId = adata.SalesOrderId;
    //            obj.Qty = adata.Quantity;
    //            obj.SubCategoryName = adata.SubCategoryName;
    //            obj.itemW8 = adata.Quantity * adata.PackageWeight;
    //            obj.crtnW8 = obj.noOfCrtn * adata.ContainerWeight;
    //            obj.Measurement = adata.ContainerSize;

    //            $scope.packingInfo.TotalCarton = Number($scope.packingInfo.TotalCarton) + Number(obj.noOfCrtn);

    //            var res = (obj.SubCategoryName).match(/Barcode Ribbon/);
    //            if (res) {
    //                var itmW8 = obj.itemW8;
    //                $scope.packingInfo.RibonNetWeight =
    //                    (Number($scope.packingInfo.RibonNetWeight) + Number(itmW8)).toFixed(2);
    //                var RibbonGrsWeight = obj.itemW8 + obj.crtnW8;
    //                $scope.packingInfo.RibonGrossWeight =
    //                    (Number($scope.packingInfo.RibonGrossWeight) + Number(RibbonGrsWeight)).toFixed(2);

    //            } else {
    //                var lblitmW8 = obj.itemW8;
    //                $scope.packingInfo.LabelNetWeight =
    //                    (Number($scope.packingInfo.LabelNetWeight) + Number(lblitmW8)).toFixed(2);
    //                var LabelGrsWeight = obj.itemW8 + obj.crtnW8;
    //                $scope.packingInfo.LabelGrossWeight =
    //                    (Number($scope.packingInfo.LabelGrossWeight) + Number(LabelGrsWeight)).toFixed(2);
    //            }
    ...
    function LoadItem(aItem) {
        ...
        //var itemName = splitItem[0].trim();
        //var desOne = splitItem[1].trim();
        //var desTwo = splitItem[2].trim();
        //var rollPerCarton = splitItem[4].trim();
        //var hsCode = splitItem[5].trim();
        var itemCode = splitItem[3].trim();
        var itemId = splitItem[7].trim();

        angular.forEach($scope.ItemSearchList, function (aData) {

            // itemName == aData.ItemName && desOne == aData.ItemDescription && desTwo == aData.ItemDescriptionTwo && rollPerCarton == aData.UnitPerPackage && hsCode == aData.HsCode && 
