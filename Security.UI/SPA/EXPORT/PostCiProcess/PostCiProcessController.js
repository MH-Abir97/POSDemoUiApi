app.controller("PostCiProcessController", function ($scope, $cookieStore, $http, $filter, $window) {
    ...
    function ClearForCC() {
        ...
        //$scope.ConsumptionCertificate.BillOfEntryDate = null;
        //$scope.ConsumptionCertificate.EpzPermissionDate = null;
   ...
   //==========Abir====>>
    ...
    function GetUpdateCIForTC() {
        ...
        if ($scope.TruckChallanList.length != 0 && $scope.CommercialInvoiceListForTC.length != 0) {
            ...
            //angular.forEach($scope.TruckChallanUniqueList, function (aTruckChallanUnique) {

            //    var item = $scope.CommercialInvoiceListForTC.find(item => item.CommercialInvoiceId === aTruckChallanUnique.CommercialInvoiceId);

            //    CINumbers.push(item.CommercialInvoiceNo);
            //});
            //for (i = 0; i < $scope.TruckChallanUniqueList.length; i++) {
            //    $scope.TruckChallanUniqueList[i].CommercialInvoiceNo = CINumbers[i];
            //}
    ...
    $scope.SaveTruckChallan = function () {
        ...
        //var res1 = $scope.LoginUser.UpdateDate.substring(0, 5);
        //if (res1 == "/Date") {
        //    var parsedDate1 = new Date(parseInt($scope.LoginUser.UpdateDate.substr(6)));
        //    var date1 = ($filter('date')(parsedDate1, 'dd.MM.yyyy')).toString();
        //    $scope.LoginUser.UpdateDate = date1;
        //} 
    ...
    //    $cookieStore.put("CompanyId", companyId);
    //    event.stopPropagation();
    ...
    $scope.SupplierLedgerReportBtn = function (SupplierLedger) {
        $window.open("#/SupplierLedgerReport", "popup", "width=850,height=550,left=280,top=80");
        var SupplierLedgerObj = SupplierLedger.CommercialInvoiceId;
        $cookieStore.put("CommercialInvoiceId", SupplierLedgerObj);
        event.stopPropagation();
    ...
    function GetConsumptionCertificateForDrop() {
        $http({
            ...
            if (data) {
                angular.forEach(data, function (e) {
                    ...
                    if (e.IsSubmitted == true && e.DocStatus != 'New Approval Pending') {
                        //   $scope.CommercialInvoiceList.push(e);
        ...
        // GetAllConsumptionCertificatePaged();
    }
    ...
    //function GetAllConsumptionCertificatePaged(curPage) {
    ...
    //    if ($scope.SearchInvoiceAndCompanyNameCC !== undefined && $scope.SearchInvoiceAndCompanyNameCC != null && $scope.SearchInvoiceAndCompanyNameCC != "") {
    ...
    //    $http({
    //        url: encodeURI('/CertificateOfOrigin/GetConsumptionCertificatePaged?startRecordNo=' + startRecordNo + '&rowPerPage=' + $scope.PerPage + "&whereClause=" + SearchCriteria + '&rows=' + 0),
    //        method: 'GET',
    //        headers: { 'Content-Type': 'application/json' }
    //    }).success(function (data) {
    //        $scope.ConsumptionCertificateList = data.ListData;
