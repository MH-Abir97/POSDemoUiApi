            ...
            angular.forEach(data, function (adata) {
                ...
                angular.forEach($scope.sockQtyWithRequestQty, function (data) {
                    if (adata.ItemAddAttId == data.ItemId) {
                        ...
                        //$scope.DepartmentList.push({ 'DepartmentName': data.DepartmentName });
                        //$scope.CurrentStockList.push({ 'CurrentQuantity': data.CurrentQuantity })
                    }

                });
                //if (currentQty.length == 0 && Department.length==0) {

                //    Department.push({ 'DepartmentName': 'Out of Stock' });
                //    currentQty.push({ 'CurrentQuantity': 0 });
