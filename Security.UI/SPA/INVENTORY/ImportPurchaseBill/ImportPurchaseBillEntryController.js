app.controller("ImportPurchaseBillEntryController", function ($scope, $cookieStore, $http, $filter, $timeout, $window) {
    ...
    $("#txtDateOfPB1").datepicker({
        ...
        onSelect: function () {
            ...
            $http({
                ...
                $scope.inv_PurchaseBill.PBNo = "IPB/"+ $scope.getAllYear+"/"+ parseInt(data);
    ...
    $scope.GetByCurrentStockWithItemId = function (itemId) {
        $http({
            url: '/StockValuation/CurrentStockGetById?ItemId=' + itemId.ItemId,
            method: 'GET',
            headers: { 'Content-Type': 'application/json' }
        }).success( function (data) {
            $scope.CurrentQuantityList = data;
            var totalCurrentStockItem = 0;
            angular.forEach($scope.CurrentQuantityList, function (aData) {       
                totalCurrentStockItem += aData.CurrentQuantity;
            });
            $scope.ItemCombination.CurrentStock = totalCurrentStockItem;
        });
    ...
    $scope.AddPBDetail = function () {
        ...
        if (flag) {
           ...
           //var  currencyId= $scope.ddlCurrencyType;
            if ($scope.ddlCurrencyType.Id ==1) {
                Attribute.CurrencyType = "BDT";
            } else {
            ...
            //})
          //  Attribute.CurrencyType = { Id: $scope.ddlCurrencyType }
            ...
            var discountAmount = 0;
             Attribute.DiscountAmount = $scope.ItemCombination.DiscountAmount;
            ...
            if (flag) {
               ...
               // Item.TotalPBPrice = TotalPBPriceCal;
                ...
                Item.DiscountAmount = Attribute.DiscountAmount;
                Item.TotalCost = Attribute.TotalCost;
                ...
                angular.forEach($scope.inv_PurchaseBillDetaillst, function (adata) {
                    ...
                    if (adata.DiscountAmount == null || adata.DiscountAmount == undefined) {
                        $scope.TotalEInClusiveCost += adata.TotalCost;
                        discountCal = adata.PBPrice;
                    ...
                    $scope.TotaInclusiveprice = $scope.TotalEInClusiveCost
                });
        ...
        alertify.confirm("Are you sure to save?", function (e) {
            if (e) {
                ...
                if ($scope.inv_PurchaseBill.AdditionDiscount != undefined && $scope.inv_PurchaseBill.AdditionDiscount != null) {
                    ...
                    $scope.inv_PurchaseBill.TotalAmountAfterDiscount = $scope.TotaInclusiveprice - additionalAfterDiscount;
                    $scope.inv_PurchaseBill.AdditionDiscount;

                    $scope.inv_PurchaseBill.TotalAmount = $scope.TotaInclusiveprice;
             
                } else {
                    $scope.inv_PurchaseBill.TotalAmountAfterDiscount = $scope.TotaInclusiveprice;
                    $scope.inv_PurchaseBill.TotalAmount = $scope.TotaInclusiveprice;
                    $scope.inv_PurchaseBill.AdditionDiscount;
                }
               ...
               // $scope.inv_PurchaseBill.PBDate = f;
                $.ajax({
                    ...
                    contentType: "application/json;charset=utf-8",
                    type: "POST",
                    data: JSON.stringify({ ainv_PurchaseBill: $scope.inv_PurchaseBill, inv_PurchaseBillDetailLst: pbItemList, inv_PurchaseBillDetailSerial: warrentyAndSerialList }),
                    success: function (data) {
                        if (data > 0) {
    ...
    $scope.itemCombinationCalculation = function (HsCodeId) {
        $scope.hsCodeConvertVal = HsCodeId;
    ...
    function ItemInsideCalculation() {
        var CDTotal = (($scope.PBsum) * ($scope.ItemCombination.CdPercentage / 100)).toFixed($scope.convertNum);
        $scope.ItemCombination.CdAmount = Number(CDTotal);

        var RDTotal = 0;
        RDTotal = (($scope.PBsum) * ($scope.ItemCombination.RdPercentage / 100)).toFixed($scope.convertNum);
        $scope.ItemCombination.RdAmount = Number(RDTotal);
        

        var SDTotal = 0;
        var cdAndRd = Number($scope.ItemCombination.CdAmount) + Number($scope.ItemCombination.RdAmount);
        SDTotal = (($scope.PBsum + cdAndRd) * ($scope.ItemCombination.SdPercentage / 100)).toFixed($scope.convertNum);
        $scope.ItemCombination.SdAmount = Number(SDTotal);



        var TotalVat = 0;
        // $scope.inv_PurchaseBillDetail.VatPercentage = adata.VAT;
        var cdAndRdWithSd = Number($scope.ItemCombination.CdAmount) + Number($scope.ItemCombination.RdAmount) + Number($scope.ItemCombination.SdAmount);
        TotalVat = (($scope.PBsum + cdAndRdWithSd) * ($scope.ItemCombination.VatPercentage / 100)).toFixed($scope.convertNum);
        $scope.ItemCombination.VatAmount = Number(TotalVat);

        var TotalAIT = 0;
        //$scope.inv_PurchaseBillDetail.AitPercentage = adata.AIT;
        TotalAIT = (($scope.PBsum) * ($scope.ItemCombination.AitPercentage / 100)).toFixed($scope.convertNum);
        $scope.ItemCombination.AitAmount = Number(TotalAIT);

        var TotalAT = 0;
        var cdAndRdWithSD = Number($scope.ItemCombination.CdAmount) + Number($scope.ItemCombination.RdAmount) + Number($scope.ItemCombination.SdAmount);
        TotalAT = (($scope.PBsum + cdAndRdWithSD) * ($scope.ItemCombination.AtPercentage / 100)).toFixed($scope.convertNum);
        $scope.ItemCombination.AtAmount = Number(TotalAT);

        var TotalTTI = 0;
        var cdAndRdSdVatATAiT = Number($scope.ItemCombination.CdAmount) + Number($scope.ItemCombination.RdAmount) + Number($scope.ItemCombination.SdAmount) + Number($scope.ItemCombination.VatAmount) + Number($scope.ItemCombination.AtAmount) + Number($scope.ItemCombination.AitAmount);
        TotalTTI = ((cdAndRdSdVatATAiT) * ($scope.ItemCombination.TtiPercentage / 100)).toFixed($scope.convertNum);
        $scope.ItemCombination.TtiAmount = Number(TotalTTI);

        $scope.ItemCombination.TotalCost = Number($scope.ItemCombination.VatAmount) + $scope.ItemCombination.PurchaseUnitPrice;
       ...
       // $scope.itemList = [];
        angular.forEach($scope.HsCodeList, function (adata) {
            if ($scope.hasCode == adata.HsCodeId) {
                ...
                var CDTotal = 0;
                var CDTotal = (($scope.PBsum) * (adata.CD / 100)).toFixed($scope.convertNum);
              
                $scope.ItemCombination.CdAmount = Number(CDTotal);

                var RDTotal = 0;
              
                RDTotal = (($scope.PBsum) * (adata.RD / 100)).toFixed($scope.convertNum);
                $scope.ItemCombination.RdAmount = Number(RDTotal);

                var SDTotal = 0;
                var cdAndRd = Number(CDTotal) + Number(RDTotal);
                SDTotal = (($scope.PBsum + cdAndRd ) * (adata.SD / 100)).toFixed($scope.convertNum);
                $scope.ItemCombination.SdAmount = Number(SDTotal);

                var TotalVat = 0;          
                var cdAndRdWithSd = Number(CDTotal) + Number(RDTotal) + Number(SDTotal);
                TotalVat = (($scope.PBsum + cdAndRdWithSd ) * (adata.VAT / 100)).toFixed($scope.convertNum);
                $scope.ItemCombination.VatAmount = Number(TotalVat);

                var TotalAIT = 0;    
                TotalAIT = (($scope.PBsum) * (adata.AIT / 100)).toFixed($scope.convertNum);
                $scope.ItemCombination.AitAmount = Number(TotalAIT);

                var TotalAT = 0;
                var cdAndRdWithSD = Number(CDTotal) + Number(RDTotal) + Number(SDTotal);
                TotalAT = (($scope.PBsum + cdAndRdWithSD) * (adata.AT / 100)).toFixed($scope.convertNum);
                $scope.ItemCombination.AtAmount = Number(TotalAT);

                var TotalTTI = 0;
                var cdAndRdSdVatATAiT = Number(CDTotal) + Number(RDTotal) + Number(SDTotal) + Number(TotalVat) + Number(TotalAT) + Number(TotalAIT);
                TotalTTI = ((cdAndRdSdVatATAiT) * (adata.TTI / 100)).toFixed($scope.convertNum);
                $scope.ItemCombination.TtiAmount = Number(TotalTTI);
               ...
               // $scope.ItemCombination.TotalCostAfterDiscount = Number(TotalTTI) + perItemDiscount;
                //$scope.TotalPriceWithDiscount = 0;
                //$scope.TotalPriceWithDiscount += (Number(TotalVat) + perItemDiscount) - $scope.inv_PurchaseBill.AdditionDiscount;

                //var PurchaseUnitPrice = 0;
                // PurchaseUnitPrice = $scope.ItemCombination.PurchaseUnitPrice - perItemDiscount;
                // $scope.ItemCombination.PurchaseUnitPrice = PurchaseUnitPrice;
            }
        });

    }

    $scope.ItemCalculationImportPB1 = function () {
        ItemInsideCalculation();
    }
    ...
    function GetPagedPB(curPage) {
        ...
        $http({
            ...
            console.log('$scope.PurchaseBillList',$scope.PurchaseBillList);
    ...
    $scope.onSelectToSupplierAndPb = function (ImportPurchaseBillList) {
        ...
        $cookieStore.put("ImportPB", ImportPurchaseBillList.SupplierId);
        $cookieStore.put("ImportPBId", ImportPurchaseBillList.PBId);
